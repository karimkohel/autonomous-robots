...
def analyse_and_map_low_clearance():
    ...
    # For Maze 3/4/5 we do not need floating-wall avoidance. Keep the
    # classification diagnostics, but do not write low-clearance cells into
    # the map, because they visually confuse debugging and can over-close
    # narrow zigzag corridors.
    if not LOW_CLEARANCE_RECORD_CELLS:
...
# Sticky-escape planner state. This is the third planning mode, separate from
# TARGET and FRONTIER. It is used only after repeated failures in the same
# local pocket, e.g. after reaching blue beside green/walls.
sticky_failure_history = []
sticky_escape_count = 0
...
def update_pose():
    ...
    if math.hypot(robot_x - last_path_x, robot_y - last_path_y) >= 0.02:
        ...
        occupancy_grid.add_traversed_metric(
            robot_x,
            robot_y,
...
def build_current_planning_layers(current_time=None, narrow_mode=False):
    """Build active planning layers.

    In normal mode this preserves the old conservative planner except for two
    competition-focused changes: low-clearance depth cells do not hard-block A*,
    and physically traversed cells soften inflated-only margins.

    In narrow mode raw walls and green remain hard blocked, but LiDAR inflation
    and temporary recovery marks are softened/ignored to handle tight zigzags.
    """
    ...
    if LOW_CLEARANCE_BLOCKS_PLANNING and not narrow_mode:
        permanent_clearance_inflated = occupancy_grid.inflate_cell_set(
            permanent_clearance_cells,
            CLEARANCE_INFLATION_RADIUS_CELLS,
        )
    else:
        # Keep cells for diagnostics/visualization, but do not feed them to A*.
        permanent_clearance_inflated = set()

    if narrow_mode and NARROW_MODE_CLEAR_TEMPORARY_RECOVERY:
        removed = occupancy_grid.clear_temporary_recovery_cells()
        if removed > 0:
            print(
                "NARROW MODE: temporary recovery cells removed before planning "
                f"({removed} cells)."
            )
        temporary_recovery_cells = set()
    else:
    ...
    if narrow_mode:
        # Aggressive narrow mode: raw LiDAR occupied cells and green remain hard
        # blocked. Normal inflation and temporary collision marks are not hard
        # blocks here; IR/LiDAR safety still protects the real robot while moving.
        inflated_cells = (
            set(occupied_cells)
            ...
            for delta_col in range(
                ...
                if (
                    ...
                    * ROBOT_START_CLEARANCE_CELLS
                    ...
                    # Never clear green or raw wall cells around the robot.
                    # Inflation/temporary margins may be cleared so the A* start
                    # cell cannot trap the robot inside a cell it already occupies.
                    if (
...
def plan_to_current_target(reason, current_time):
    ...
    if path is None:
        print(
            ...
            "Trying a sticky-escape open-region plan before ordinary frontier search."
        )

        if plan_sticky_escape(
            reason + " | target approach unavailable; escape to open region first",
            current_time,
            target_position,
        ):
            return
        ...
        print(
            f"No sticky escape was available. Exploring before retrying for at least "
...
def mark_contact_region_from_sensors(ir_values, current_time):
    ...
    if (
        ...
        or (
            LOW_CLEARANCE_STOPS_MOTION
            and last_clearance_front <= CONTACT_RECOVERY_LIDAR_DISTANCE
        )
    ):
...
def enter_collision_recovery(reason, current_time, ir_values):
    ...
    contact_confirmed = (
        ...
        or (
            LOW_CLEARANCE_STOPS_MOTION
            and last_clearance_front <= STUCK_OBSTACLE_CONFIRM_DISTANCE
        )
...
# ============================================================
# Sticky-escape planning mode
# ============================================================


def record_sticky_planning_failure(reason, current_time):
    """
    Return True if planning has failed repeatedly while the robot has stayed in
    the same small physical region.

    This is deliberately pose-based rather than reason-string based: green,
    no-frontier, and target-unreachable failures can all be symptoms of the
    same local pocket.
    """

    global sticky_failure_history

    sticky_failure_history.append(
        (current_time, robot_x, robot_y, mission_state, reason)
    )

    sticky_failure_history = [
        item
        for item in sticky_failure_history
        if current_time - item[0] <= STICKY_FAILURE_WINDOW
    ]

    recent_same_area = []
    for failure_time, failure_x, failure_y, failure_state, failure_reason in sticky_failure_history:
        if math.hypot(robot_x - failure_x, robot_y - failure_y) <= STICKY_FAILURE_RADIUS:
            recent_same_area.append(
                (failure_time, failure_state, failure_reason)
            )

    if len(recent_same_area) >= STICKY_FAILURE_COUNT:
        print("\n----------------------------------------")
        print("STICKY LOCAL POSE DETECTED")
        print(
            f"Failures in {STICKY_FAILURE_RADIUS:.2f} m radius: "
            f"{len(recent_same_area)}/{STICKY_FAILURE_COUNT}"
        )
        print(f"Latest reason: {reason}")
        print("A third planning mode will try to escape to a nearby open region.")
        print("----------------------------------------\n")
        return True
...
def local_region_scores(cell, inflated_cells):
    """Return (free_count, unknown_count, blocked_count) around a cell."""

    row, col = cell
    radius = ESCAPE_OPEN_REGION_RADIUS_CELLS
    ...
    for delta_row in range(-radius, radius + 1):
        for delta_col in range(-radius, radius + 1):
            ...
            test = (row + delta_row, col + delta_col)
            if not occupancy_grid.is_inside(*test):
                blocked_count += 1
                continue

            if test in inflated_cells:
                blocked_count += 1
                continue

            state = occupancy_grid.cell_state(*test)
            if state == "free":
                free_count += 1
            elif state == "unknown":
                unknown_count += 1
...
def choose_sticky_escape_goal(robot_cell, inflated_cells, preferred_target_position=None):
    """
    Pick a reachable open-space goal near the robot.

    This is not a frontier and not a pillar approach. It is a local escape pose:
    a free cell in a wide-enough nearby region that A* can reach from the
    current trapped pose. If a target position is known, prefer escape poses
    that still make some progress toward it; otherwise prefer open poses near
    unknown edges so later scans can reveal more map.
    """

    if robot_cell is None:
        return None, None, float("inf"), float("inf")

    min_cells = max(1, int(round(ESCAPE_MIN_DISTANCE / MAP_RESOLUTION)))
    max_cells = max(min_cells + 1, int(round(ESCAPE_MAX_DISTANCE / MAP_RESOLUTION)))
    preferred_cells = max(min_cells, int(round(ESCAPE_PREFERRED_DISTANCE / MAP_RESOLUTION)))

    target_cell = None
    if preferred_target_position is not None:
        target_cell = occupancy_grid.metric_to_grid(
            preferred_target_position[0],
            preferred_target_position[1],
        )

    candidates = []

    for row in range(1, occupancy_grid.size - 1, ESCAPE_CANDIDATE_STRIDE_CELLS):
        for col in range(1, occupancy_grid.size - 1, ESCAPE_CANDIDATE_STRIDE_CELLS):
            cell = (row, col)
            if cell in inflated_cells:
                continue
            if occupancy_grid.cell_state(row, col) != "free":
                continue

            distance_cells = euclidean_cells(robot_cell, cell)
            if distance_cells < min_cells or distance_cells > max_cells:
                continue

            free_count, unknown_count, blocked_count = local_region_scores(
                cell,
                inflated_cells,
            )

            if free_count < ESCAPE_MIN_FREE_CELLS:
                continue

            # Pre-score before expensive A*. Lower is better.
            score = 0.0
            score += ESCAPE_DISTANCE_PREFERENCE_WEIGHT * abs(
                distance_cells - preferred_cells
            )
            score -= ESCAPE_OPEN_REGION_REWARD * math.sqrt(free_count)

            if target_cell is not None:
                score += ESCAPE_TARGET_DISTANCE_WEIGHT * euclidean_cells(
                    cell,
                    target_cell,
                )
            else:
                # When the next pillar is unknown, prefer open regions near
                # unknown boundaries. This is a gentler version of frontier
                # exploration that can work even when frontier clusters are too
                # small to be selected as official frontiers.
                score -= ESCAPE_UNKNOWN_EDGE_REWARD * math.sqrt(unknown_count)

            # Penalize crowded pockets.
            score += 0.015 * blocked_count

            candidates.append((score, cell, free_count, unknown_count))

    candidates.sort(key=lambda item: item[0])
    candidates = candidates[:ESCAPE_MAX_CANDIDATES_TO_TEST]

    best = None

    for preliminary_score, candidate_cell, free_count, unknown_count in candidates:
        path, path_cost = occupancy_grid.astar(
            robot_cell,
            candidate_cell,
            inflated_cells,
        ...
        total_score = preliminary_score + path_cost
...
def plan_sticky_escape(reason, current_time, preferred_target_position=None):
    """
    Create an ESCAPE plan to a nearby open region.

    Returns True if a plan was created and execution entered FOLLOW_PATH.
    """
    ...
    global planning_attempts, latest_planning_output_path
    global active_plan_type, active_target_colour
    global sticky_escape_count, sticky_escape_reason
    global sticky_escape_start_x, sticky_escape_start_y
    global green_confirmation_count, emergency_ir_confirmation_count

    if sticky_escape_count >= MAX_STICKY_ESCAPES:
        return False
    ...
    (
        ...
        current_time,
        narrow_mode=NARROW_MODE_ENABLED,
    )

    escape_goal, path, path_cost, selection_score = choose_sticky_escape_goal(
        robot_cell,
        inflated_cells,
        preferred_target_position,
    )

    if path is None:
        print("\n========================================")
        print("STICKY ESCAPE PLAN FAILED")
        print("----------------------------------------")
        print(f"Reason: {reason}")
        print("No reachable open-region escape cell was found.")
        print("Falling back to the older safe-explore behaviour if available.")
        print("========================================\n")
        return False

    planning_attempts += 1
    sticky_escape_count += 1
    sticky_escape_reason = reason
    sticky_escape_start_x = robot_x
    sticky_escape_start_y = robot_y

    selected_cluster = None
    selected_goal = escape_goal
    planned_astar_path = path
    planned_path_cost = path_cost
    planned_selection_score = selection_score
    ...
    goal_x, goal_y = occupancy_grid.grid_to_metric(*selected_goal)
    moved_goal_distance = math.hypot(goal_x - robot_x, goal_y - robot_y)

    print("\n========================================")
    print("STICKY ESCAPE PLAN CREATED")
    print("----------------------------------------")
    print(f"Trigger               : {reason}")
    print(f"Mission state          : {mission_state}")
    print(
        f"Robot physical width   : {ROBOT_ESTIMATED_TOTAL_WIDTH:.2f} m | "
        f"planning radius={ROBOT_PLANNING_RADIUS_METRES:.2f} m "
        f"({ROBOT_PLANNING_RADIUS_CELLS} cells)"
    )
    print(
        f"Escape goal            : x={goal_x:.2f}, y={goal_y:.2f} "
        f"({moved_goal_distance:.2f} m away)"
    )
    print(
        f"A* escape path length  : {planned_path_cost * MAP_RESOLUTION:.2f} m"
    )
    print(f"Simplified waypoints   : {len(planned_waypoints)}")
    print(f"Planning map           : {latest_planning_output_path}")
    print("After reaching this open pose, normal target/frontier planning resumes.")
    print("========================================\n")
    ...
    first_x, first_y = planned_waypoints[0]
    print(
        "Starting sticky escape navigation | "
        f"first waypoint=({first_x:.2f}, {first_y:.2f})"
    )
...
def handle_safe_explore(current_time, front_ir, ir_values, green_check_updated):
    ...
    obstacle_close = (
        ...
        or (
            LOW_CLEARANCE_STOPS_MOTION
            and last_clearance_front <= SAFE_EXPLORE_OBSTACLE_DISTANCE
        )
    )
...
def handle_no_reachable_frontier(reason, current_time):
    ...
    if (
        NARROW_MODE_ENABLED
        and record_sticky_planning_failure(reason, current_time)
        and plan_sticky_escape(
            reason + " | repeated no-frontier failures; narrow escape",
            current_time,
            None,
        )
    ):
...
def follow_planned_path(
    ...
    if abs(heading_error) > TURN_IN_PLACE_THRESHOLD:
        ...
        turn_speed = PATH_TURN_SPEED
        if (
            active_plan_type == "ESCAPE"
            or min(ir_values) < NARROW_CORRIDOR_SIDE_DISTANCE
            or last_front_lidar < NARROW_CORRIDOR_FRONT_LIDAR_DISTANCE
        ):
            turn_speed *= NARROW_CORRIDOR_TURN_SPEED_SCALE
    ...
    obstacle_ahead = (
        ...
        or (
            LOW_CLEARANCE_STOPS_MOTION
            and last_clearance_front < CLEARANCE_FRONT_STOP_DISTANCE
        )
    )
    ...
    # In narrow corridors, false close-range IR readings are common. Do not
    # immediately declare collision; instead slow the local follower down so
    # it can thread the zigzag using the existing A* route.
    if (
        active_plan_type == "ESCAPE"
        or min(ir_values) < NARROW_CORRIDOR_SIDE_DISTANCE
        or last_front_lidar < NARROW_CORRIDOR_FRONT_LIDAR_DISTANCE
    ):
        speed_scale *= NARROW_CORRIDOR_SPEED_SCALE
...
print("Low-clearance/floating-wall cells are ignored and hidden for Maze 3/4/5 run")
print("Colour-ratio arrival + LiDAR/IR wall safety + 2x speed")
print(
    "Physical footprint explicit: "
    f"width≈{ROBOT_ESTIMATED_TOTAL_WIDTH:.2f} m, "
    f"planning radius≈{ROBOT_PLANNING_RADIUS_METRES:.2f} m "
    f"({ROBOT_PLANNING_RADIUS_CELLS} cells)"
)
...
while robot.step(time_step) != -1:
    ...
    if (
        ...
        if (
            LOW_CLEARANCE_TRIGGERS_REPLAN
            and phase == "FOLLOW_PATH"
    ...
    if (
        ...
        and (
            ...
            or (
                LOW_CLEARANCE_TRIGGERS_REPLAN
                and last_clearance_new_cells > 0
            )
        )
    ):
        (
        ...
        semantic_blocked = set(forbidden_inflated_cells)
        if LOW_CLEARANCE_TRIGGERS_REPLAN:
            semantic_blocked |= clearance_inflated_cells
